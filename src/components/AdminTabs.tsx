...
export default function AdminTabs() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-[100] md:relative md:bottom-auto md:top-0 px-4 pb-8 md:pb-0 md:mb-8 pointer-events-none">
      <div className="max-w-4xl mx-auto flex items-center justify-between p-2 glass-card rounded-[2rem] md:rounded-[2.5rem] pointer-events-auto shadow-2xl shadow-black/50 border-t-white/20 border-b-white/5 border-x-white/10 backdrop-blur-3xl overflow-hidden">
        <div className="flex w-full overflow-x-auto hide-scrollbar justify-start sm:justify-around md:justify-start gap-2 px-1">
          {tabs.map((tab) => {
            ...
            return (
              <a
                key={tab.id}
                href={tab.href}
                className={`flex flex-col md:flex-row items-center gap-1 md:gap-3 px-3 py-2 min-w-[80px] md:min-w-0 md:px-6 md:py-4 rounded-[1.5rem] font-bold transition-all duration-300 uppercase tracking-tighter text-[9px] md:text-xs flex-shrink-0
                  ${isActive 
